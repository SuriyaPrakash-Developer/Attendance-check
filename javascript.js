...
btn.addEventListener("click", () => {
  ...
  if (isNaN(present) || isNaN(total) || isNaN(percentage)) {
    return (outputDiv.innerText = "Error ? Enter the value");
  }

  if (present < 0 || total <= 0 || present > total) {
    return (outputDiv.innerText = "No.of present Days");
  }
...
const daysToAttendClassText = (attendanceNeeded, present, total, percentage) =>
  `You need to attend <strong>${attendanceNeeded}</strong> more classes ${percentage}% <br>Current Attendance: <strong>${present}/${total}</strong> ->  <strong>${(
    (present / total) *
    100
